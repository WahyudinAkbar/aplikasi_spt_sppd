...
class AddSptPage extends StatefulWidget {
  const AddSptPage({super.key});

  @override
  State<AddSptPage> createState() => _AddSptPageState();
}

class _AddSptPageState extends State<AddSptPage> {
  ...
  void AddSptPage() async {
    QuerySnapshot idUsers = await FirebaseFirestore.instance
        .collection("users")
        .where('nama', isEqualTo: selectedValuePegawai)
        .get();
    try {
      await FirebaseFirestore.instance.collection('spt').add({
        'no_spt': noSuratController.text,
        'id_users': idUsers.docs[0].reference.id,
